...
The product bulk-upload (Inventory → Bulk Upload) accepts a pharmacy's master-data columns directly via case-insensitive header aliases: Code→barcode, Material Name→name, Unit→main unit, Small Unit→pack_size, Quantity→stock, Sales Price→price. The older headers (barcode, name_en, name_ar, stock, price, …) still work too.
