...
type LabelSize = 'thermal' | 'thermal_tall'
...
function buildStyles(size: LabelSize): string {
  const cfg = SIZE[size]
  return `
...
export default function DoseLabelPrint({ items, onClose, defaultSize = 'thermal_tall' }: Props) {
  ...
  const openPrint = async (dialog: boolean) => {
    const valid = rows.filter((r) => r.doseText.trim() && r.qty > 0)
    if (!valid.length) {
    ...
    try {
      ...
      if (!w) {
        alert(t('dose_labels.popup_blocked'))
        return
      }
      w.document.title = t('dose_labels.print_title')
      ...
      title.textContent = t('dose_labels.print_toolbar_title')
      const btn = w.document.createElement('button')
      btn.type = 'button'
      btn.textContent = t('dose_labels.print_toolbar_btn')
      btn.onclick = () => { w.focus(); w.print() }
      const hint = w.document.createElement('span')
      hint.textContent = t('dose_labels.print_toolbar_hint')
